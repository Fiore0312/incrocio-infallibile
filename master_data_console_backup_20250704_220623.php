...
try {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as employee_count FROM dipendenti WHERE attivo = 1");
    $stmt->execute();
    $employee_count = $stmt->fetch()['employee_count'];
    
    $stmt = $conn->prepare("SELECT COUNT(*) as company_count FROM clienti");
    $stmt->execute();
    $company_count = $stmt->fetch()['company_count'];
    
    $stmt = $conn->prepare("SELECT COUNT(*) as vehicle_count FROM veicoli WHERE attivo = 1");
    $stmt->execute();
    $vehicle_count = $stmt->fetch()['vehicle_count'];
    ...
    $stmt = $conn->prepare("
        SELECT id, nome, cognome, CONCAT(nome, ' ', cognome) as nome_completo, email, 
               'dipendente' as ruolo, costo_giornaliero, attivo,
               0 as legacy_links
        FROM dipendenti 
        WHERE attivo = 1
    ...
    if (empty($employees)) {
        ...
        echo "<strong>Lista Fissa:</strong> Questi sono i $employee_count dipendenti certi della tua organizzazione. ";
